...
fun setImageViewResource(imageView: ImageView, resource: String) {
    Glide.with(imageView.context).load(resource).apply(RequestOptions.circleCropTransform()).into(imageView)
}
...
fun setImageViewResourceCountry(imageView: ImageView, resource: String) {
    ...
    if(flag!=0) {
        Glide.with(imageView.context).load(flag).apply(RequestOptions.circleCropTransform()).into(imageView)
    } else {
